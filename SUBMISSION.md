...
### 在线体验（推荐）
直接打开：https://jvbaoge1.github.io/safe-wallet/

### 本地部署
```bash
git clone https://github.com/jvbaoge1/safe-wallet.git
cd safe-wallet
npm install
npm run dev
```
...
### 操作流程
1. 打开页面 → 等待 tcx-wasm 初始化完成（约2秒）
2. 输入密码 → 点击「创建新钱包」
3. 查看多链地址（ETH/TRX/BTC + SOL/SUI）
4. 在 Wallet 标签 → 测试转账 UI（含 Gas 费和风险评估）
5. 切换到 Security 标签 → 测试 URL 安全扫描
6. 切换到 Tools 标签 → 测试消息签名
